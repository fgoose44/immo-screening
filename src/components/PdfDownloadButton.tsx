...
export default function PdfDownloadButton({ propertyId, initialPdfUrl }: Props) {
  ...
  if (loading) {
    return (
      <button
        disabled
        className="flex items-center gap-2 px-3 py-1.5 text-[13px] rounded-[8px] bg-surface-disabled text-content-muted cursor-not-allowed"
      >
    ...
    return (
      <div className="flex flex-col items-end gap-1">
        <a
          ...
          className="flex items-center gap-2 px-3 py-1.5 text-[13px] rounded-[8px] bg-brand-primary text-white hover:bg-brand-primary-dark transition-colors font-medium"
        >
  ...
  return (
    <div className="flex flex-col items-end gap-1">
      <button
        onClick={handleGenerate}
        className="flex items-center gap-2 px-3 py-1.5 text-[13px] rounded-[8px] border border-border text-content-secondary hover:border-brand-primary hover:text-brand-primary transition-colors"
      >
        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
